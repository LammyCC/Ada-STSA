...
from model.Ada_STGL.AdaSTGL_base import TFTrainer, Trainer
from model.Ada_STGL.adj_mx import get_adj


class AdaSTGLStepTrainer(Trainer):
    """
    ...
    Args:
        ...
        model_graph_fusion: Model for graph fusion.
    ...
    def __init__(self, model_pred, model_graph, optimizer_pred, optimizer_graph, scheduler_pred,
        ...
        adj_mx_list = self.add_adj_mx_list()
        ...
        self.model_graph_trainer = self.MacroLearnTrainer(model_graph, optimizer_graph, scheduler_graph, epoch_num, scaler, self.graph_save_path, self)
    ...
    def add_adj_mx_list(self):
        # init adj matrix
        adj_mx = torch.diag(torch.rand(self.node_num))
        adj_mx = adj_mx.to(dtype=torch.float32, device=self.device)
        init_adj_mx = [adj_mx]
        return init_adj_mx
    ...
    class ModelPredTrainer(TFTrainer):
        def __init__(self, model, optimizer, lr_scheduler, max_epoch_num, scaler, model_save_path, outer_obj):
            ...
            self.outer_obj: AdaSTGLStepTrainer = outer_obj
            self.num_iter: int = 1
        ...
        def train_one_epoch(self, data_loader):
            ...
            for _ in range(self.num_iter):
                for x_enc, x_enc_mark, x_dec, x_dec_mark, traj_emb, y in data_loader:
                    ...
                    traj_emb = traj_emb.type(torch.float32).to(self.device)
                    y = y.type(torch.float32).to(self.device)
                    adj_mx = self.outer_obj.best_adj_mx.to(x_enc.device)
                    pred = self.model(x_enc, x_enc_mark, x_dec, x_dec_mark, adj_mx=adj_mx, traj_emb=traj_emb)
    ...
    class MacroLearnTrainer(TFTrainer):
        def __init__(self, model, optimizer, lr_scheduler, max_epoch_num, scaler, model_save_path, outer_obj):
            super().__init__(model, optimizer, lr_scheduler, max_epoch_num, scaler, model_save_path)
            self.outer_obj: AdaSTGLStepTrainer = outer_obj
            self.num_iter: int = 1
        ...
        def train_one_epoch(self, data_loader):
            ...
            for _ in range(self.num_iter):
                for x_enc, x_enc_mark, x_dec, x_dec_mark, traj_mx, y in data_loader:
                    ...
                    adj_mx = adj_mx.to(x_enc.device)
                    pred = self.outer_obj.model_pred(x_enc, x_enc_mark, x_dec, x_dec_mark, traj_mx, adj_mx=adj_mx)
                    loss = F.l1_loss(pred, y)
                    self.optimizer.zero_grad()
                    loss.backward()
                    self.optimizer.step()
        ...
        def evaluate(self, data_loader, metrics=('mae', 'rmse', 'mape')):
            ...
            num = 0
            for x_enc, x_enc_mark, x_dec, x_dec_mark, traj_mx, y in data_loader:
                new_adj_mx = self.model(self.outer_obj.best_adj_mx).detach()
                adj_mx += new_adj_mx
                num += 1
            adj_mx = adj_mx / num
